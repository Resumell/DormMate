...
#define VOICE_TX_PIN    11  
#define VOICE_RX_PIN    12  
...
void voice_send_cmd(uint8_t msg_id)
{
    ...
    uart_wait_tx_done(VOICE_UART, pdMS_TO_TICKS(100));
}
...
void voice_task(void *pv)
{
    ...
    while (1) {
        int len = uart_read_bytes(VOICE_UART, buf, VOICE_BUF_SIZE, pdMS_TO_TICKS(100));
        if (len > 0) {
            // ===== 加这行：打印原始数据 =====
            ESP_LOGI(TAG, "UART原始数据(%d字节):", len);
